...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	default:
		...
		switch elem[0] {
		case '/': // Prefix: "/shortener"

			if l := len("/shortener"); len(elem) >= l && elem[0:l] == "/shortener" {
				elem = elem[l:]
			} else {
				break
			}
			...
			case '/': // Prefix: "/"
				...
				if len(elem) == 0 {
					...
					switch r.Method {
					case "DELETE":
						s.handleDeleteShortUrlRequest([1]string{
							args[0],
						}, elemIsEscaped, w, r)
					case "GET":
						s.handleGetFullUrlRequest([1]string{
							args[0],
						}, elemIsEscaped, w, r)
					default:
						s.notAllowed(w, r, notAllowedParams{
	...
	default:
		...
		case '/': // Prefix: "/shortener"
			...
			if len(elem) == 0 {
				switch method {
				case "PUT":
					r.name = CreateShortUrlOperation
					r.summary = "Создать короткий URL"
					r.operationID = "createShortUrl"
					r.operationGroup = ""
					r.pathPattern = "/shortener"
					r.args = args
					r.count = 0
					return r, true
				default:
			...
			case '/': // Prefix: "/"
				...
				if len(elem) == 0 {
					...
					switch method {
					case "DELETE":
						r.name = DeleteShortUrlOperation
